...
trait EAV {
	...
	{
		return isset($this->metadataFields) ? $this->metadataFields : [];
	}

	/**
	 * Metadata validation methods
	 */
	public function validateMetadata(array $metadata, $validFields = null)
	{
		list($fields, $values) = [array_keys($metadata), array_values($metadata)];

		return $this->validateMetadataFields($fields, $validFields) and $this->validateMetadataValues($data);
	}

	protected function validateMetadataFields(array $fields, $validFields = null)
	{
		$validFields = is_array($validFields) ? $validFields : $this->getMetadataFields();

		return count(array_intersect($fields, $validFields)) === count($fields)
		   and count(array_diff($fields, $validFields)) === 0;
	}

	protected function validateMetadataValues(array $values)
	{
	...
	{
		$this->processCreatedMetadata($metadata);
	}
	...
	protected function processUpdatedMetadata(array $metadata = array())
	{
		...
		{
			if($property = $this->metadata->getItemByField($field))
			{
				$property->value = $val;
				$property->save();
			}
